  ...
  <head>
        ...
        $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var careerOptionsHtml = careerOptionsTemplate({ career: data["countries"] });
    ...
    <script>
        $(document).ready(function() {
            ...
            $.ajax({
                url: "http://localhost:8000/api/index/reports",
                type: "GET",
                headers: {
                ...
                success: function(data) {
                    ...
                    var careerAsksHtml = careerAsksTemplate({ asks: data["data of asks in your country"] });
...
<script>$(document).ready(function() {
            ...
            if (!token) {
                ...
                "image": $("#fileInput").val(),
                "country": $("#careerInput").val(),
                };
                var formDataJSON = JSON.stringify(formData);
                console.log( $("#careerInput").val());

                $.ajax({
                    url: "http://localhost:8000/api/report", // استبدل هذا بعنوان نقطة نهاية التسجيل الفعلية
                    type: "POST",
                    headers: {
...
<script>
     function handleKeyPress(event) {
            ...
            if (event.key === 'Enter') {
                ...
                $.ajax({
                    url: "http://localhost:8000/api/reports/search", // استبدل هذا بعنوان نقطة نهاية التسجيل الفعلية
                    type: "POST",
                    headers: {
                        "Accept": "application/json",
                        "Content-Type": "application/json",
                    ...
                    success: function (response) {
                        ...
                        var careerAsksSource = $("#asks-template").html();
                            var careerAsksTemplate = Handlebars.compile(careerAsksSource);
                            var careerAsksHtml = careerAsksTemplate({ asks: response["data of asks in your country"] });
                            $("#posts-container").html(careerAsksHtml);
                    },
                    error: function (error) {
...
<script>
        function upVote(upDiv) {
            ...
            console.log(token);
                ...
                fetch(`http://localhost:8000/api/reports/ups/${postId}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
